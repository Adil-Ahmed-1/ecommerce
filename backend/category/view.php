...
if (isset($_GET['delete'])) {
    $did = (int)$_GET['delete'];
    $cat = mysqli_fetch_assoc(mysqli_query($conn, "SELECT image FROM categories WHERE id = $did"));
    if ($cat && !empty($cat['image']) && file_exists('uploads/' . $cat['image'])) {
...
<aside id="sidebar" class="sidebar-glass fixed left-0 top-0 h-full w-[260px] text-white z-50 transition-all duration-300 flex flex-col">
  ...
  <nav class="flex-1 mt-2 px-3 space-y-1 overflow-y-auto">
    ...
    <a href="category/view.php" class="nav-link active flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium"><i class="fa-solid fa-layer-group w-5 text-center text-[13px]"></i><span class="sidebar-text transition-all duration-300">View Categories</span></a>
